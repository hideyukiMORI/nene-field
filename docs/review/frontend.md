...
- [ ] **Security:** auth token in `sessionStorage` via `createSessionTokenStore` (fleet AU-1), **never `localStorage`**; fail-closed (401→login, 403→forbidden); RBAC gating is UX only; no `dangerouslySetInnerHTML` without DOMPurify+Issue; `rel="noopener noreferrer"` on `target="_blank"`; no secrets (only `VITE_*`).

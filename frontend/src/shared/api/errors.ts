/**
 * RFC 9457 `application/problem+json` body shape (the fields the client reads).
 * Defined inline rather than imported from the generated schema so transport
 * error handling never depends on a specific generated type name.
 */
...
/**
 * The single error type thrown across the client boundary. Built from a Problem
 * Details body (or a transport failure). Features map `slug` / `status` to
 * user-facing messages — they never parse responses themselves.
 */
export class AppError extends Error {
  ...
  static fromProblem(status: number, body: unknown): AppError {
    const problem: ProblemDetailsBody = typeof body === 'object' && body !== null ? body : {}

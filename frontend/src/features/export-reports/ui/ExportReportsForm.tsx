    ...
    <div className="mx-auto w-full max-w-5xl">
      ...
      <div className="mb-6 flex flex-wrap items-end gap-4.5 border-b border-border pb-4.5">
        <div className="min-w-0">
          <p className="text-xs font-bold tracking-wide text-on-accent">{t('export.kicker')}</p>
          <h2 className="mt-2 text-doc-title font-bold tracking-tight text-text-primary">
            {t('export.title')}
          </h2>
          <p className="mt-1.5 max-w-xl text-sm text-x-fg-muted-2">{t('export.subtitle')}</p>
        </div>
        <div className="flex-1" />
        <Button onClick={onDownload} disabled={!canExport} className="flex-none whitespace-nowrap">
          ⬇ {t('export.download')}
          <span className="ml-2 rounded-x-pill bg-text-inverse/20 px-2 py-0.5 tabular-nums">
            {matched.length}
          </span>
        </Button>
      </div>
      ...
      <div className="flex flex-col items-start gap-5 lg:flex-row">
        ...
        <div className="flex w-full flex-col rounded-2xl border border-border bg-surface-raised p-5.5 lg:w-90 lg:flex-none">
          <h3 className="text-sm font-bold text-text-primary">{t('export.preview.title')}</h3>
          <p className="mt-0.5 text-xs text-x-fg-faint-2">{t('export.preview.lead')}</p>
          <p className="mt-3.5 mb-4.5 flex items-baseline gap-1.5">
            <span className="text-stat font-bold leading-none text-on-accent tabular-nums">
              {matched.length}
            </span>
            <span className="text-sm text-x-fg-muted-2">件</span>
          </p>

          <p className="mb-2 text-xs font-semibold tracking-wide text-x-fg-faint-2">
            {t('export.preview.leading')}
          </p>
          <div className="flex flex-col gap-2">
            {matched.length === 0 ? (
              <p className="text-sm text-text-faint">{t('export.preview.empty')}</p>
            ) : (
          ...
          <p className="mt-4.5 border-t border-dashed border-border pt-3.5 text-center text-xs leading-relaxed text-text-faint">

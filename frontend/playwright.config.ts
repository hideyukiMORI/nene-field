...
// Default 5191 (NeNe Field's 5190 dev port + 1, to avoid clashing with a running
// dev server). Override with E2E_PORT.
 ...
 * tests are deterministic and focus on UI behaviour (auth, validation,
 * navigation). The in-memory token is lost on reload, so specs log in through
 * the form and then navigate within the SPA (see e2e/helpers/auth.ts).
 */
